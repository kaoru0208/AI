...
model = tf.keras.models.load_model(
    str(pathlib.Path.home() / "projects/fxbot/lstm_fx.h5"), compile=False
)
...
def market_open(retry=3) -> bool:
    for _ in range(retry):
        try:
            r = prc.PricingInfo(accountID=ACCOUNT_ID, params={"instruments": PAIR})
            api.request(r)
...
def latest(n=60):
    r = ins.InstrumentsCandles(
        instrument=PAIR, params={"count": n, "granularity": "M1", "price": "M"}
    )
    api.request(r)
...
def units_now():
    r = pos.OpenPositions(accountID=ACCOUNT_ID)
    api.request(r)
...
def close_all(side):  # "longUnits" / "shortUnits"
    api.request(
        PositionClose(accountID=ACCOUNT_ID, instrument=PAIR, data={side: "ALL"})
    )
    print("✔ CLOSE", side)
...
def discord(msg):
    os.system(
        f'curl -s -H "Content-Type: application/json" -X POST '
...
def send_order(side, prob):
    ...
    data = {
        "order": {
            "units": str(LOT if side == "BUY" else -LOT),
            "instrument": PAIR,
            "type": "MARKET",
