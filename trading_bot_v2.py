...
try:
    ...
    RISK_PER_TRADE = (
        config.RISK_PER_TRADE if hasattr(config, "RISK_PER_TRADE") else 0.01
    )
    ...
    NEWS_CHECK_INTERVAL = (
        config.NEWS_CHECK_INTERVAL if hasattr(config, "NEWS_CHECK_INTERVAL") else 3600
    )
except ImportError:
...
if not ACCOUNT_ID or not ACCESS_TOKEN:
    raise RuntimeError(
        "Missing OANDA API credentials! Please set them in config.py or environment variables."
    )
    ...
    while True:
        ...
        if news_client and (time.time() - last_news_check > NEWS_CHECK_INTERVAL):
            ...
            if sentiment_score is not None:
                ...
                print(
                    f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')} - News sentiment updated (score={news_sentiment:.3f})"
                )
        ...
        if signal:
            ...
            if news_sentiment is not None:
                if signal == "BUY" and news_sentiment < -0.3:
                    print(
                        f"シグナル=BUY しかしニュースセンチメントがネガティブ（score={news_sentiment:.2f}）、取引を見送り。"
                    )
                    signal = None
                elif signal == "SELL" and news_sentiment > 0.3:
                    print(
                        f"シグナル=SELL しかしニュースセンチメントがポジティブ（score={news_sentiment:.2f}）、取引を見送り。"
                    )
                    signal = None
            if not signal:
            ...
            try:
                ...
                print(
                    f"[WARN] 口座残高の取得に失敗しました。デフォルト値を使用します: {e}"
                )
            ...
            stop_distance = (
                atr if atr > 0 else (0.001 if INSTRUMENT.endswith("JPY") else 0.0001)
            )
            units = calculate_position_size(
                balance if balance is not None else 10000,
                RISK_PER_TRADE,
                stop_distance,
                current_price,
            )
            if units == 0:
                print(
                    "[INFO] 計算されたポジションサイズが0のため、取引をスキップします。"
                )
                continue
            if signal == "SELL":
            ...
            try:
                trailing_distance = (
                    stop_distance  # トレーリングストップ幅としてATR相当を使用
                )
                ...
                opened_trades += 1
                print(
                    f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')} - {INSTRUMENT} に対して {signal} の成行注文発注: 数量={units} (注文ID: {resp.get('orderFillTransaction', {}).get('id', 'N/A')})"
                )
            except Exception as e:
        ...
        if opened_trades >= 5:
            print(
                "5件の注文を発注したため、デモ用リミットに達しました。ボットを停止します。"
            )
            break
